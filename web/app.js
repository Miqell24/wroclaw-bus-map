// Frontend: MapLibre GL + OpenFreeMap vector tiles (positron) + MPK line layers
...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: GTFS UM Wrocław (odt.org.pl) — MPK and the agglomeration carriers' }));
  ...
  const paintChips = (linesView) => {
    document.getElementById('chips').innerHTML = meta.lines
      .map((l) => `<button class="chip${l.line === state.selected && l.mode === state.selMode ? ' active' : ''}" data-line="${esc(l.line)}" data-mode="${esc(l.mode)}" ` +
        `style="background:${esc(linesView ? lineColor(l.line) : l.color)}">${esc(l.label || l.line)}</button>`)
      .join(' ');
  };
  ...
  // modes — here no number is used twice at all, and the pair carries the
  // guarantee anyway.
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: UM Wrocław';
      ...
      a.download = `wroclaw-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `wroclaw-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the zone lines run out to Trzebnica, Oleśnica,
  // Oława and Sobótka and the data bbox is 56 × 42 km, so fitting it would
  // open on fields. Open on the city, the inner ring inside the frame.
  map.jumpTo({ center: [17.035, 51.108], zoom: 11.2 });
}
