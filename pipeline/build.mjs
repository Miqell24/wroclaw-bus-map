...
// Wrocław: ONE feed on the sheet — the city's own GTFS (UM Wrocław, mirrored
// by odt.org.pl), split by route_type into buses (3) and trams (0). It carries
// the WHOLE agglomeration in one bundle: MPK's city buses and trams, and the
// suburban and zone lines the private carriers run for the county communes —
// DLA Wrocław, PT KŁOSOK and NOWAK TRANSPORT — which is where the high numbers
// come from. The feed's own classification (route_types.txt) reads: 56 normal
// bus lines, 26 trams, 4 express (A, D, K, N), 3 suburban (602, 607, 612), 31
// zone lines (903…967), 17 night (206, 240…259) and one seasonal (715).
// No metro and no suburban-rail slot: the tram network is the whole rail side.
// Line keys are the operator's OWN designations, unchanged; no number is used
// twice on this sheet, so none of them needs an operator prefix.
...
// Night buses are the 2xx family: 206 and 240…259 (the feed's own type 40).
const NIGHT = /^2\d\d$/;
...
// ONE feed, TWO cfgs: routeTypes splits the bundle per mode (3 = bus,
// 0 = tram). Without the filter `--all` on the bus mode would swallow the
// trams too.
//
// Nothing is skipped: every route in this feed is a passenger line — the city
// publishes its depot runs as trips, not as routes.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/wroclaw.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['100']),
  feeds: [
    { tag: 'mpk', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['3'] },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
// No rail trunk in Wrocław: nothing gets the metro treatment (wide ribbon,
// station discs, always-on names) — the tram network is the rail side, and
// the tourist line T is a tram like any other.
const isRailTrunk = () => false;
if (tramAll || tramLines.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/wroclaw-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramLines,
  feeds: [
    { tag: 'mpk', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['0'] },
  ],
});
...
await (await import('./night.mjs')).nightPass(outDir, /^2\d\d$/, { sort: true });
...
// A no-op while every line here takes its mode's colour.
await (await import('./railrows.mjs')).railRowPass(outDir, /^2\d\d$/, { log });
